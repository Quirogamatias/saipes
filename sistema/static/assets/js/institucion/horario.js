...
function listadoHorariosT(){
    $.ajax({
        url: "/institucion/listar_horariot/",
        type: "get",
        dataType: "json",
        success: function(response){
            if($.fn.DataTable.isDataTable('#tabla_horariost')){
                $('#tabla_horariost').DataTable().destroy();
            }
            $('#tabla_horariost tbody').html("");
            for(let i = 0;i < response.length;i++){
                let fila = '<tr>';
                fila += '<td>' + (i+1) + '</td>';
                fila += '<td>' + response[i]["fields"]['dia'] + '</td>';
                ...
                fila += '</tr>';
                $('#tabla_horariost tbody').append(fila);
            }             
            $('#tabla_horariost').DataTable({
                language: {
                    "decimal": "",
                    "emptyTable": "No hay información",
                    "info": "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
