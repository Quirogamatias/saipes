function listadoAsistencias(){
    $.ajax({
        url: "/institucion/listar_asistencias/",
        type: "get",
        dataType: "json",
        success: function(response){
            ...
            for(let i = 0;i < response.length;i++){
                ...
                fila += ' onclick = "abrir_modal_edicion(\'/institucion/editar_asistencia/' + response[i]['pk']+'/\');"> EDITAR </button>';
                fila += '<button type = "button" class = "btn btn-danger tableButton btn-sm"';
                fila += 'onclick = "abrir_modal_eliminacion(\'/institucion/eliminar_asistencia/'+ response[i]['pk']+'/\');"> ELIMINAR </button> </td>';
                fila += '</tr>';
                $('#tabla_asistencias tbody').append(fila);
            }             
...
function eliminar(pk){
	$.ajax({
		...
		url: '/institucion/asistencia/eliminar_asistencia/'+pk+'/',
		type: 'post',
		success: function(response) {
            notificacionSuccess(response.mensaje);
			listadoAsistencias();
